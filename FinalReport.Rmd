...
income_query <- "select distinct acs_year, es_short, median_household_income from census.income_by_catchment"


df_income <- dbGetQuery(connec, income_query)
df_income$acs_year <- as.factor(df_income$acs_year)

ggplot(df_income, aes(x=acs_year, y=median_household_income)) +
  coord_flip() +
  geom_boxplot()
...
deltas_query <- "select
	...
	ratio.year_2,
	ratio.es_id,
	cast(ratio.year_1 as text) || '-' || cast(ratio.year_2 as text) as years_delta,
	income.median_household_income_delta as income_delta,
	ratio.ratio_delta
from public.ratio_delta ratio
inner join
	census.income_deltas income
	on
		ratio.es_id = income.es_id"


df_deltas <- dbGetQuery(connec, deltas_query)
head(df_deltas)


ggplot(df_deltas, aes(x=years_delta, y=ratio_delta, group = years_delta, color=years_delta)) +
  coord_flip() +
  geom_boxplot()

ggplot(df_deltas, aes(x=years_delta, y=income_delta, group = years_delta, color=years_delta)) +
  coord_flip() +
  geom_boxplot()


ggplot(df_deltas, aes(x=ratio_delta, y=income_delta, color=years_delta)) +
  geom_point()

```

There seems to be clustering around zero change.
