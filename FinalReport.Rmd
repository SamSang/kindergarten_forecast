...
#head(df_full_compare)
pairs(df_full_compare[c(3:9,11:12)])
...
But there's serious hetroscedasticity between births and enrollment.

There's no clear correlation between the year to year changes, unfortunately.
...
df.income <- df_full_compare[c(1,2,3,7)]
...
df.charter <- df_full_compare

df.charter$next_to_charter <- as.factor(df.charter$next_to_charter)

ggplot(df.charter, aes(x=next_to_charter, y=ratio, fill=catchment_year)) +
  coord_flip() +
  geom_boxplot()
